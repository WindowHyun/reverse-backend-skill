...
**1-A2. 요청/응답 스키마**: 각 엔드포인트의 **요청**(바디 필드·타입·필수 여부, 쿼리
...
**1-B2. 응답 민감 필드 노출 diff**: 응답으로 나가는 필드(1-A2)를 모델 필드와 대조해,
...
**1-C2. 인증 플로우 도달성(end-to-end)**: 가드(`requireAuth` 등)만 보지 말고 **토큰
...
**1-I. 실행 가능성(runnability) 신호**: "이 코드가 지금 실제로 도는 물건인가"를 판정할
...
**1-J. 완성처럼 보이지만 미완성인 것(스텁 인벤토리)**: 이름·시그니처는 완성돼 있으나 본문이
...
1. Document Overview
   ...
   1.4 실행 가능성 판정 (부팅 가능/불가 + 근거; DB연결·env로딩·필수키·고아파일) — 1-I
   1.5 스텁/미완성 인벤토리 ("완성처럼 보이나 미완성인 것") — 1-J
2. Scope & API Policy
   2.1 API 엔드포인트 맵 (라우트 열거 교차검증 결과 포함 시 표기)
   2.2 데이터 모델
   2.3 공통 정책 (인증/인가, 에러 처리 공통 규칙)
   2.4 엔드포인트별 정책 + 요청/응답 스키마 (바디 필드·쿼리 파라미터·성공 응답 필드) — 1-A2
...
5. 보안 점검 ★ (문서 최상단에도 요약 배치 / OWASP API Security Top 10 2023 매핑)
   ...
   5.4 응답 민감 필드 노출 · 비즈니스 로직/인젝션 결함 (API3/API6 등; 1-B2·Step2) — 확정 등급으로
